...
from assistant.voice.bridge import VoiceBridgeOrchestrator, ConversationState
...
def mock_voice_components(mock_moshi):
    ...
    with patch('assistant.voice.bridge.MoshiBridge') as MockMoshi, \
         patch('assistant.voice.bridge.AIClient') as MockAIClient, \
         patch('assistant.voice.bridge.ConversationLoop') as MockConversationLoop:
...
class TestStateManagement:
    ...
    async def test_initial_state_is_idle(self, orchestrator_factory, mock_moshi):
        ...
        with patch('assistant.voice.bridge.MoshiBridge', return_value=mock_moshi):
            await orchestrator.initialize()
