...
impl Dashboard {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        // Start audio visualizer listener (always run, even in dev mode)
        // This provides real-time audio visualization in the dashboard
        let audio_handle = match self.spawn_audio_visualizer_listener() {
            Ok(handle) => {
                info!("Audio visualizer listener spawned successfully");
                Some(handle)
            }
            Err(e) => {
                warn!("Failed to start audio visualizer listener: {}", e);
                // Add error to dashboard but continue
                {
                    let mut state = self.state.write().await;
                    state.add_initialization_error(InitializationError::AudioDevice {
                        message: format!("Audio visualizer failed to start: {}", e),
                    });
                }
                None
            }
        ...
        tokio::spawn(async move {
            ...
            match crate::voice::VoiceBridge::new(voice_config.clone()).await {
                Ok(voice_bridge) => {
                    ...
                    // Create local audio system for voice conversation
                    // This runs in a blocking task because CPAL streams are not Send
                    ...
                    tokio::task::spawn_blocking(move || {
                        ...
                        rt.block_on(async {
                            info!("Creating local audio system for voice conversation");

                            // Create local audio system
                            let (mut audio_system, audio_rx, playback_tx) = match crate::local_audio::LocalAudioSystem::new() {
                                Ok(system) => system,
                                Err(e) => {
                                    error!("Failed to create local audio system for conversation: {}", e);
                                    let mut state = state_for_conversation.write().await;
                                    state.add_event(ActivityEvent::Error {
                                        message: format!("Failed to create audio system: {}", e),
                                        time: Local::now(),
                                    });
                                    return;
                                }
                            ...
                            // Start BOTH input (microphone) and output (speakers)
                            if let Err(e) = audio_system.start() {
                                error!("Failed to start local audio system: {}", e);
                                let mut state = state_for_conversation.write().await;
                                state.add_event(ActivityEvent::Error {
                            ...
                            info!("Local audio system started (microphone + speakers)");

                            // Start voice conversation loop (connects microphone → MOSHI → speakers)
                            match voice_bridge_for_conversation.start_local_conversation(audio_rx, playback_tx).await {
                                Ok(handle) => {
            ...
            Ok(voice_bridge) => {
                ...
                tokio::task::spawn_blocking(move || {
                    ...
                    rt.block_on(async {
                        ...
                        // Create local audio system
                        let (mut audio_system, audio_rx, playback_tx) = match super::local_audio::LocalAudioSystem::new() {
                            Ok(system) => system,
                            Err(e) => {
                                error!("Failed to create local audio system for conversation: {}", e);
                                let mut state = state_for_conversation.write().await;
